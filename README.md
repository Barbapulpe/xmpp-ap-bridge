...
Finally, you need to create an application to retrieve a secret toke. Do the following:
...
You can use environment variables for locating the configuration file and for the bot accounts credentials. These can be set in the .env file, and if storing credentials, permissions should be restricted with `chmod 600 .env`. Delete unused lines in the .env file.
...
| XMPP_BRIDGE_NAME | Full account handle of the Mastodon bot, format `name@example.net`. If not set, name in configuration file will be used. |
| XMPP_BRIDGE_TOKEN | Access token of the Mastodon bot. If not set, token in configuration file will be used. |
| AP_BRIDGE_JID | Full JID (account name) of the XMPP bot, format `name@example.net`. If not set, name in configuration file will be used. |
...
If you choose to store the credentials there, make sure permissions are restricted with `chmod 600 /path/to/config/file/filename`
...
- Several files are used for logs, database, Bridge status and translation messages, each directory used be customized.
- Some of the key Bridge functional parameters are defined here, which cannot be modified by the administrators.
- All commands and prefixes can be changes, although the latter might need code review on regex depending on the changes.
...
On the first run, the Bridge will create and initialize all requires files and database tables. On subsequent runs, cleanup is performed on each startup: you should consider a regular restart of the backend bots.
...
Conversely, this induces limitations on scalability, mostly imposed by the servers. XMPP host server may limit communications in various way, as there are many different configurations out there. Mastodon host server has a hard-coded rate-limit of 300 API calls per 5 minutes (an average of 1 call per second).

So in a scenario with many users, this rate-limiting might cause delay in forwarding the messages. This is why you are encouraged to deploy your own Bridge if you intend to use it thoroughly, in the spirit of federation.
...
- Sending and receiving XMPP and Fediverse server administrators can read messages (as with any other non-encrypted message).
...
This software is licensed under the GNU Affero General Public License v3.0 which is provided with the source code, and is available here: https://www.gnu.org/licenses/agpl-3.0.en.html#license-text
